...
Do not call update_goal unless the goal is actually complete.`);
	...
	it("includes codex audit guidance in objective-updated steer messages", () => {
		...
		expect(prompt).toContain("Blocked audit:");
		expect(prompt).toContain(
			"Only mark the updated goal achieved when current evidence proves every requirement has been satisfied and no required work remains.",
		);
		expect(prompt).toContain(
			"Only use status \"blocked\" when the same blocking condition has repeated for at least three consecutive goal turns",
		);
	...
	it("includes codex work guidance in objective-updated steer messages", () => {
		const goal = {
			...makeGoalSnapshot("ship <better> & verify", 500, null, "2026-05-02T00:00:00.000Z"),
			tokensUsed: 125,
		};
		...
		expect(prompt).toContain("Fidelity:");
		expect(prompt).toContain(
			"Keep the full updated objective intact. If it cannot be finished now, make concrete progress toward the real requested end state",
		);
		expect(prompt).toContain(
			"Use the current worktree and external state as authoritative.",
		);
		expect(prompt).toContain(
			"Treat alignment as movement toward the updated requested end state.",
		);
	});
...
- Preserve the updated objective's full scope; do not redefine success around the work that already exists.
- For every explicit requirement, numbered item, named artifact, command, test, gate, invariant, and deliverable, identify the authoritative evidence that would prove it, then inspect the relevant current-state sources: files, command output, test results, PR state, rendered artifacts, runtime behavior, or other authoritative evidence.
- For each item, determine whether the evidence proves completion, contradicts completion, shows incomplete work, is too weak or indirect to verify completion, or is missing.
- Match the verification scope to the requirement's scope; do not use a narrow check to support a broad claim.
- Treat tests, manifests, verifiers, green checks, and search results as evidence only after confirming they cover the relevant requirement.
...
Do not call update_goal unless the updated goal is complete or the strict blocked audit above is satisfied. Do not mark a goal complete merely because the budget is nearly exhausted or because you are stopping work.`);
	});
});
