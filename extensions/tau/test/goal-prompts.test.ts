...
Do not call update_goal unless the goal is complete or the strict blocked audit above is satisfied. Do not mark a goal complete merely because the budget is nearly exhausted or because you are stopping work.`);
	...
	it("renders configured time budgets in active goal context", () => {
		...
		const updatedPrompt = objectiveUpdatedPrompt(goal);
		expect(updatedPrompt).not.toContain("- Time spent pursuing goal:");
		expect(updatedPrompt).not.toContain("- Time budget:");
		expect(updatedPrompt).not.toContain("- Time remaining:");
		expect(updatedPrompt).not.toContain("goal.timeBudgetSeconds");
	});
...
Do not call update_goal unless the goal is actually complete.`);
	...
	it("keeps objective-updated steer messages short like codex", () => {
		const goal = {
		...
		expect(prompt).not.toContain("Blocked audit:");
		expect(prompt).not.toContain("Continuation behavior:");
		expect(prompt).not.toContain("Work from evidence:");
	});

	it("renders unbudgeted objective-updated remaining tokens like codex", () => {
		const goal = {
			...makeGoalSnapshot("ship <better> & verify", null, null, "2026-05-02T00:00:00.000Z"),
			tokensUsed: 125,
		};
