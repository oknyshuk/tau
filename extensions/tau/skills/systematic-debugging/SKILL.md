...
**Action:** Use `read` on the relevant source files. Use `bash` with `rg` to find the error string in the codebase.
...
vitest run test/module.test.ts -t "specific behavior"

# Run with verbose output
vitest run test/module.test.ts --reporter=verbose
```
...
**WHEN system has multiple components (HTTP → service → database, CLI → worker → API):**
...
**Action:** Use `bash` with `rg` to trace references:

```bash
# Find where the function is called
rg -n "functionName\\(" src test

# Find where the variable is set
rg -n "variableName\\s*=" src test
```
...
- [ ] Problem isolated to specific component or code
...
**Action:** Use `bash` with `rg` to find comparable patterns:

```bash
rg -n "similarPattern" src test
```
...
vitest run test/module.test.ts -t "regression"
...
```text
agent spawn deep "Investigate why [specific test/behavior] fails
